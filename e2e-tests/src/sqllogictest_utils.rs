...
impl std::fmt::Display for SQLLogicTestEntry {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Command: {}, Expectation: {}",
            self.command, self.expectation
        )
    }
}
...
pub fn start_server(config: &'static ServerConfig) -> thread::JoinHandle<()> {
    thread::spawn(move || {
